...
xss_count, sql_count, lfi_count, xss_payload, lfi_payload, sql_payload, host, link = check_vulnerabilities(session, url_list, host)
...
pdf.cell(30, ch, f'Time of the report : {date}', 0)
pdf.ln(ch)
pdf.ln(ch)
...
if ((xss_count + sql_count + lfi_count) != 0) :
...
pdf.ln(ch)
pdf.set_font('Arial', '', 12)
for i in range(0, xss_count):
    pdf.multi_cell(0, 5, f"We found one or more XSS vulnerabilities on this link : {host}{link}, this is the payload used", 0)
    pdf.set_text_color(r= 255, g= 0, b = 0)
    pdf.multi_cell(0, 5, f"{xss_payload}")
    pdf.set_text_color(r= 0, g= 0, b = 0)
pdf.ln(ch)
for i in range(0, lfi_count):
    pdf.multi_cell(0, 5, f"We found one or more LFI vulnerabilities on this link : {host}{link}, this is the payload used", 0)
    pdf.set_text_color(r= 255, g= 0, b = 0)
    pdf.multi_cell(0, 5, f"{lfi_payload}")
    pdf.set_text_color(r= 0, g= 0, b = 0)
pdf.ln(ch)
for i in range(0, sql_count):
    pdf.multi_cell(0, 5, f"We found one or more SQL vulnerabilities on this link : {host}{link}, this is the payload used", 0)
    pdf.set_text_color(r= 255, g= 0, b = 0)
    pdf.multi_cell(0, 5, f"{sql_payload}")
    pdf.set_text_color(r= 0, g= 0, b = 0)
pdf.ln(ch)
...
if  ((xss_count + sql_count + lfi_count) != 0):
    ...
    if  (xss_count != 0):
        ...
        pdf.ln(2)
        pdf.multi_cell(w=0, h=5, txt="https://capec.mitre.org/data/definitions/63.html")
        pdf.multi_cell(w=0, h=5, txt="https://www.chiny.me/attaque-xss-cross-site-scripting-14-3.php")
    ...
    if  (lfi_count != 0):
        ...
        pdf.ln(2)
        pdf.multi_cell(w=0, h=5, txt="https://www.invicti.com/web-vulnerability-scanner/vulnerabilities/local-file-inclusion/")
    ...
    if  (sql_count != 0):
        ...
        pdf.ln(2)
        pdf.multi_cell(w=0, h=5, txt="https://www.softwaresecured.com/introduction-to-sql-injection-mitigation/")
        pdf.multi_cell(w=0, h=5, txt="https://www.chiny.me/l-injection-sql-14-2.php")
